...
class GenerateInvoice with PrintPDF {
  ...
  static Future<String> printInvoice(
    ...
    final path = await PrintPDF.getTemporaryPdfFilePath(
        fileName: invoiceModel.transactionId.toString());
  ...
  static pw.Widget buildPDFHeader({required InvoiceModel invoiceModel}) {
    final headerData = [
      ...
      pw.TableRow(
        children: [
          pw.Text(
            'Mobile: ${invoiceModel.mobile.toString()}',
            style: const pw.TextStyle(fontSize: 7),
            textAlign: pw.TextAlign.center,
          ),
        ],
  ...
  static pw.Widget buildPDFSectionInvoice(
    ...
    final data = [
      pw.TableRow(children: [
        ...
        pw.Spacer(),
        invoiceModel.invoiceNumber != null
            ? pw.Text(
                invoiceModel.invoiceNumber?.toString() ?? 'Offline Invoice No.',
                style: const pw.TextStyle(fontSize: 7))
            : pw.Text(invoiceModel.offlineInvoiceNo?.toString() ?? '',
                style: const pw.TextStyle(fontSize: 7)),
      ]),
      pw.TableRow(children: [
  ...
  static pw.Widget productTableFooter({required InvoiceModel invoiceModel}) {
    final double changedReturn =
        double.parse(invoiceModel.changeReturn ?? '0.0');
    ...
    String stringTax = (invoiceModel.taxPercentage.toString())
        .replaceAll(RegExp(r'[^\d.]'), '');
    double totalTax = double.parse(double.parse(stringTax).toStringAsFixed(2));
    ...
    return pw.Table(
      ...
      children: [
        pw.TableRow(children: [
          ...
          pw.Text(
            '\$ ${_formatNumber(double.parse(invoiceModel.subTotal ?? '0.0').toStringAsFixed(2))}',
            style: const pw.TextStyle(fontSize: 7),
          ),
        ]),
        pw.TableRow(children: [
          ...
          pw.Text(
            '(-) ${_formatNumber(double.parse(invoiceModel.invoiceDiscount!).toStringAsFixed(2))}',
            style: const pw.TextStyle(fontSize: 7),
          ),
        ]),
        pw.TableRow(children: [
        ...
        pw.TableRow(children: [
          ...
          pw.Text(
            '\$ ${_formatNumber(double.parse(invoiceModel.totalPaid!.toString()).toStringAsFixed(2))}',
            style: const pw.TextStyle(fontSize: 7),
          ),
        ]),
        pw.TableRow(children: [
          ...
          pw.Text(
            '\$ ${_formatNumber(double.parse(invoiceModel.balance ?? '0.0').toStringAsFixed(2))}',
            style: const pw.TextStyle(fontSize: 7),
          ),
        ]),
        if (changedReturn > 0.0)
          pw.TableRow(children: [
            pw.Text(
              'Change Return',
              style: const pw.TextStyle(fontSize: 7),
            ...
            pw.Text(
              '\$ ${_formatNumber(double.parse(invoiceModel.changeReturn.toString()).toStringAsFixed(2))}',
              style: const pw.TextStyle(fontSize: 7),
            ),
          ]),
      ],
  ...
  static pw.Widget productTableView({required InvoiceModel invoiceModel}) {
    final data = List.generate(invoiceModel.product!.length, (index) {
      ...
      if (product.unit_price != null && product.quantity != null) {
        ...
        log('Product unit price or quantity is null.');
      }

      double unitPriceDiscount =
          product.lineDiscountAmount != null && product.lineDiscountAmount != ""
              ? FunctionProduct.applyDiscount(
              ...
              : 0.0; // If no discount, set discount to 0.

      // Log unit price and discount information
      log('Product: ${product.name}');
      log('Unit Price: ${product.unit_price}');
      log('Unit Price Discount: $unitPriceDiscount');

      // Calculate effective unit price
      double effectiveUnitPrice = unitPriceDiscount;
      product.calculate != null
          ? double.parse(product.calculate.toString())
          : double.parse(unitPriceDiscount.toString());
      (double.parse(product.unit_price.toString()) - unitPriceDiscount);

      // Log effective unit price
      log('Effective Unit Price for ${product.name}: $effectiveUnitPrice');

      if (effectiveUnitPrice == 0) {
        log('Warning: Effective price is zero for ${product.name}.');
      }
      return pw.TableRow(
        ...
        children: [
          ...
          pw.Text(
            '${_formatNumber(unitPriceDiscount.toString())}',
            style: const pw.TextStyle(fontSize: 7),
          ),
          pw.Text(
            '${_formatNumber((effectiveUnitPrice * int.parse(product.quantity.toString())).toStringAsFixed(2))}',
            style: const pw.TextStyle(fontSize: 7),
          ),
        ],
      );
