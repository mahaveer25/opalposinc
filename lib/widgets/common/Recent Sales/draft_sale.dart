...
class DraftSale extends StatefulWidget {
  const DraftSale({super.key});

  @override
  State<DraftSale> createState() => _DraftSaleState();
}

class _DraftSaleState extends State<DraftSale> {
  ...
  onInit() async {
    try {
      final List<RecentSalesModel> data =
          await recentSale.getRecentSales(context, type = 'draft');
      setState(() {
        recentModel = data;
        isLoading = false;
      });
