...
import 'package:opalposinc/CloverDesign/Dashboard%20Pages/Add%20Item%20Inventory%20V1/Add%20Item/add_item_inventory.dart';
...
class _Refundv1State extends State<Refundv1> with PrintPDF {
    ...
    showDialog(
        ...
        builder: (context) {
          return InvoiceReturnPopup(
            returnInvoiceController: popUpInvoiceController,
            onPressed: () async {
              await getSellReturn(
                popUpInvoiceController,
              );
    ...
    try {
      ...
      if (returnsale != null) {
        ...
        discountController.text =
            double.parse(returnsale?.discountAmount ?? "0.0")
                .toStringAsFixed(2);
        date.text = returnsale?.date ?? "";
        returnsale?.product?.forEach((_) {
          returnQuantityControllers.add(TextEditingController());
        });
        cashTotal = double.parse(
            CommonFunctions.getCashTotalInReturn(returnsale ?? InvoiceModel()));
        cardTotal = double.parse(
            CommonFunctions.getCardTotalInReturn(returnsale ?? InvoiceModel()));
        controller.clear();
  ...
  double invoiceDiscount({required List<Product> productList}) {
    ...
    final discount =
        double.parse(returnsale?.discountAmount.toString() ?? "0.0");
  ...
  double taxInvoice({required double discount}) {
    String stringTax = (returnsale?.taxPercentage.toString())
  ...
  void calculateCashCardReturnAmount() {
    setState(() {
      ...
      final cashTotal = double.parse(
          CommonFunctions.getCashTotalInReturn(returnsale ?? InvoiceModel()));
      final cardTotal = double.parse(
          CommonFunctions.getCardTotalInReturn(returnsale ?? InvoiceModel()));
                                    ...
                                    SingleChildScrollView(
                                      ...
                                      child: DataTable(
                                        ...
                                        rows: List<DataRow>.generate(
                                            returnsale?.product?.length ?? 0,
                                            (index) {
                                          Product product =
                                              returnsale?.product?[index] ??
                                                  Product();
    ...
    if (cardTotal != 0.0 && cardAmountController.text != "0.00") {
      if (paxDeviceBloc.state != null) {
        final response = await BridgePayService.postBridgePay(
            ...
            tenderType:
                CommonFunctions.getCardType(returnsale ?? InvoiceModel()) ??
                    "CREDIT",
            transType: "REFUND");
