...
//                        ).show()
                        ...
                        showMotionToast("Printer not Connected", "error")
                        result.error("PRINTER_NOT_CONNECTED", "Printer is not connected.",  mapOf("isPrinterConnected" to false))
    ...
    // Handle USB reconnection events
        ...
        override fun onReceive(context: Context, intent: Intent) {
            ...
            when (action) {
                ...
                UsbManager.ACTION_USB_DEVICE_DETACHED -> {
                    val device = intent.getParcelableExtra<UsbDevice>(UsbManager.EXTRA_DEVICE)
                    if (device != null && device == mUSBDevice) {
    ...
    private fun reconnectPrinter() {
        val usbManager = getSystemService(USB_SERVICE) as UsbManager
        doDiscovery(usbManager)
        if (deviceList.isNullOrEmpty()) {
            //Toast.makeText(mContext, "No printer found", Toast.LENGTH_SHORT).show()
            showMotionToast("No printer found ", "error")
            return
        }

        mUSBDevice = deviceList!![0]
        myPrinter = PrinterInstance.getPrinterInstance(mContext, mUSBDevice, mHandler)
        devicesName = mUSBDevice!!.deviceName

        showMotionToast("Printer Reconnect id $devicesName ", "success")
        //Toast.makeText(mContext, "id: $devicesName", Toast.LENGTH_SHORT).show()
    ...
    private class MyHandler(activity: MainActivity , private val showMotionToast: (String, String) -> Unit) : Handler() {
...
//                    Toast.makeText(activity?.mContext, R.string.conn_failed, Toast.LENGTH_SHORT)
//                        .show()
                    showMotionToast("Connect Failed","error")
                    XLog.i(activity?.TAG, "ZL at SettingActivity Handler() 连接失败!")
                }
                ...
                Connect.CLOSED -> {
                    ...
                    GlobalContants.ISCONNECTED = activity?.isConnected == true
                    GlobalContants.DEVICENAME = activity?.devicesName
                    //Toast.makeText(activity?.mContext, R.string.conn_closed, Toast.LENGTH_SHORT)
                    showMotionToast("Printer Connection Close" ,"error")
                    //

                    XLog.i(activity?.TAG, "ZL at SettingActivity Handler() 连接关闭!")
                }

                Connect.NODEVICE -> {
                    activity?.isConnected = false
                    //Toast.makeText(activity?.mContext, R.string.conn_no, Toast.LENGTH_SHORT).show()
                    showMotionToast("No Device Can Connect" ,"error")
                }
    ...
    private val mHandler: Handler = MyHandler(this){ message, toastType ->
        showMotionToast(message, toastType)
    }
    ...
    fun usbAutoConn(manager: UsbManager) {
        ...
        myPrinter = PrinterInstance.getPrinterInstance(mContext, mUSBDevice, mHandler)
        devicesName = mUSBDevice!!.deviceName
        //Toast.makeText(mContext, devicesName, Toast.LENGTH_SHORT).show()
        showMotionToast("Printer Connected id  $devicesName", "success")
    ...
    fun printPdf(path: String?) {
        if (path == null) {
            //Toast.makeText(this, getString(R.string.choose_pdf_file), Toast.LENGTH_SHORT).show()
            showMotionToast("choose pdf file","error")
            return
        }
        ...
        runOnUiThread {
            ...
            Log.i(
                "mainactivity",
                "zl --getAllFiles()----fileAdapter.add(f.getAbsolutePath()): $path"
            )
        }


        //Toast.makeText(this, "", Toast.LENGTH_SHORT).show()
        // Ensure MotionToast runs on the UI thread
        Log.d(
            "TAG",
            " Before Printing"
        )
        showMotionToast("Printing", "success")
                    ...
                    while (count < pageCount) {
                        ...
                        if (bitmap!!.width > PrinterConstants.paperWidth) {
                            bitmap = Utils.zoomImage(
                                ...
                                PrefUtils.getInt(applicationContext, GlobalContants.PRINTERTYPE, 0)
                            )
                        }
                        bitmap = Utils.zoomImage(
                            bitmap,
    ...
    // ✅ Fixed function placement (moved outside configureFlutterEngine)
    private fun showMotionToast(message: String, toastType: String) {
        val activity: Activity? = this
        activity?.runOnUiThread {
            MotionToast.createColorToast(
                activity,
                "Notification",  // ✅ Title added
                message,
                getToastStyle(toastType),  // ✅ Function call fixed
                MotionToast.GRAVITY_BOTTOM,
                MotionToast.LONG_DURATION,
                null
                // ResourcesCompat.getFont(this, R.font.helvetica_regular) // ✅ Fixed null font issue
            )
        }
    }

    private fun getToastStyle(toastType: String): MotionToastStyle {
        return when (toastType.lowercase()) {
...
//                        ).show()
                        ...
                        result.error(
                            ...
                            mapOf("isPrinterConnected" to false)
                        )
                    }
                } else {
                        result.error("UNAVAILABLE", "Path not available.", null)
